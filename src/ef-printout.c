...
void ef_printip(IPhdr *ip) {
    ...
    if ((ip->ip_off & IP_RF) >> 15) STPUTS("RESERVED ");
    if ((ip->ip_off & IP_DF) >> 14) STPUTS("DF ");
    if ((ip->ip_off & IP_MF) >> 13) STPUTS("MF ");
...
void ef_printtcp(TCPhdr *tcp) {
    ...
    if (tcp->th_flags & TH_ACK) STPUTS("ACK ");
    if (tcp->th_flags & TH_RST) STPUTS("RST ");
    if (tcp->th_flags & TH_PUSH) STPUTS("PSH ");
    if (tcp->th_flags & TH_URG) STPUTS("URG ");
    if (tcp->th_flags & TH_FIN) STPUTS("FIN ");
    if (tcp->th_flags & TH_ECE) STPUTS("ECE ");
    if (tcp->th_flags & TH_CWR) STPUTS("CWR ");
...
#pragma GCC diagnostic pop
    ...
    case 0:
        icmptype = "Echo Reply";
        if (icmp->icmp_code == 0) icmpcode = "Echo Reply";
        break;
    case 3:
    ...
    case 4:
        ...
        if (icmp->icmp_code == 0) icmpcode = "Source Quench";
        break;
    case 5:
        icmptype = "Redirect";
        switch (icmp->icmp_code) {
    ...
    case 8:
        ...
        if (icmp->icmp_code == 0) icmpcode = "Echo Request";
        break;
    case 9:
        icmptype = "Router Advertisement";
        if (icmp->icmp_code == 0) icmpcode = "Router Advertisement";
        break;
    case 10:
        icmptype = "Router Solicitation";
        if (icmp->icmp_code == 0) icmpcode = "Router Solicitation";
        break;
    case 11:
    ...
    case 13:
        ...
        if (icmp->icmp_code == 0) icmpcode = "Timestamp Request";
        break;
    case 14:
        icmpcode = "Timestamp Reply";
        if (icmp->icmp_code == 0) icmpcode = "Timestamp Reply";
        break;
    case 15:
        icmptype = "Information Request";
        if (icmp->icmp_code == 0) icmpcode = "Information Request";
        break;
    case 16:
        icmptype = "Information Reply";
        if (icmp->icmp_code == 0) icmpcode = "Information Reply";
        break;
    case 17:
        icmptype = "Address Mask Request";
        if (icmp->icmp_code == 0) icmpcode = "Address Mask Request";
        break;
    case 18:
        icmptype = "Address Mask Reply";
        if (icmp->icmp_code == 0) icmptype = "Address Mask Reply";
        break;
    default:
        icmptype = "Unknown";
        break;
    ...
    if (mode == ICMP_MASKREQ) printf(" [ICMP Address Mask] %s\n", mask);
    if (mode == ICMP_REDIRECT) printf(" [ICMP Pref Gateway] %s\n", gateway);
...
void ef_device_failure(int mode, const char *device) {
    if (mode == INJECTION_RAW)
        ...
        fprintf(stderr,
                "ERROR: Unable to open layer 2 device '%s' for packet "
                "injection: %s.\n",
                device, errbuf);
