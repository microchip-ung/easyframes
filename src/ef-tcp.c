...
int buildtcp(ETHERhdr *eth, IPhdr *ip, TCPhdr *tcp, FileData *pd,
    ...
    if (verbose == 2) ef_hexdump(pkt, tcp_packetlen, HEX_ASCII_DECODE);
    if (verbose == 3) ef_hexdump(pkt, tcp_packetlen, HEX_RAW_DECODE);
...
void ef_tcp(int argc, char **argv) {
