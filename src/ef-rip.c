...
int buildrip(ETHERhdr *eth, IPhdr *ip, UDPhdr *udp, RIPhdr *rip, FileData *pd,
    ...
    if (verbose == 2) ef_hexdump(pkt, rip_packetlen, HEX_ASCII_DECODE);
    if (verbose == 3) ef_hexdump(pkt, rip_packetlen, HEX_RAW_DECODE);
...
void ef_rip(int argc, char **argv) {
...
static void rip_validatedata(void) {
    ...
    if (riphdr.ver == 2) {
        ...
        if (riphdr.mask == 0)
            ef_name_resolve("255.255.255.0", (u_int32_t *)&riphdr.mask);
    }
    ...
    if (iphdr.ip_dst.s_addr == 0) {
        ...
        case 2:
            ...
            ef_name_resolve("224.0.0.9", (u_int32_t *)&iphdr.ip_dst.s_addr);
            break;
        default:
...
static void rip_cmdline(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, rip_options)) != -1) {
        ...
        case 'h': /* RIP next hop address */
            if ((ef_name_resolve(optarg, (u_int32_t *)&riphdr.next_hop)) <
                0) {
                fprintf(stderr,
        ...
        case 'i': /* RIP route address */
            if ((ef_name_resolve(optarg, (u_int32_t *)&riphdr.addr)) < 0) {
                fprintf(stderr,
                        "ERROR: Invalid destination IP address: "
                        "\"%s\".\n",
                        optarg);
        ...
        case 'k': /* RIP netmask address */
            if ((ef_name_resolve(optarg, (u_int32_t *)&riphdr.mask)) < 0) {
                fprintf(stderr,
