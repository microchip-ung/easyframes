 ...
 **/
int builddatafromfile(const size_t buffsize, FileData *memory, const char *file,
                      const u_int32_t mode) {
    if ((memory->file_mem = (u_int8_t *)calloc(buffsize, sizeof(u_int8_t))) ==
        NULL) {
        if (mode == PAYLOADMODE)
